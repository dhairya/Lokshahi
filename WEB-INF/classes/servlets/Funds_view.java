...
/**
 * Servlet implementation class for Servlet: Funds_view
 *
 */
 public class Funds_view extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
	        ...
	        PreparedStatement ps=con.prepareStatement("select * from "+url+" where FUND_ID=?");
	        ps.setLong(1,fundid);
	        ...
	        if(wardB==null){
				RequestDispatcher view = request
						.getRequestDispatcher("./"+priv+"/view_fundsC.jsp?consti="+constiB);
				view.forward(request, response);
				}
				else
				{
					RequestDispatcher view = request
					.getRequestDispatcher("./"+priv+"/view_funds.jsp?consti="+constiB+"&ward="+wardB);
			view.forward(request, response);
				}
