 ...
 * 
 */
public class Grievance_nxt extends javax.servlet.http.HttpServlet implements
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (sertype.equals("txtser")) {
					String ref_id1 = (String) request.getParameter("searchref1");
					String searchtxt1 = (String) request.getParameter("searchtxt1");
					//System.out.println(searchtxt1);
					if (ref_id1.equals("")) {
						ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
								"FROM "+url+" where TITLE =? ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
						...
						ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
								"FROM "+url+" where REF_ID =? ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
				...
				ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
						"FROM "+url+" where NICKNAME=? ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
				ps.setString(1,nick);
				rs = ps.executeQuery();
				request.setAttribute("row", row1);
				...
				ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
						"FROM "+url+" ORDER BY INTELLI_RATING DESC, REF_ID DESC LIMIT " + rowq + ", 20");
				rs = ps.executeQuery();
				request.setAttribute("row", row1);
				...
				ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
						"FROM "+url+" where REF_ID IN (SELECT REF_ID FROM " + url1 +
						" WHERE NICKNAME = ?) ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
				ps.setString(1,nick);
				rs = ps.executeQuery();
				request.setAttribute("row", row1);
				...
				ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
						"FROM "+url+" where "+statstr+" ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
				
				rs = ps.executeQuery();
				request.setAttribute("row", row2);
				request.setAttribute("sertype", "status");
			}else if(sertype.equals("all")){
				ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
						"FROM "+url+" ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
				rs = ps.executeQuery();
				request.setAttribute("row", row1);
				...
				ps = con.prepareStatement("SELECT REF_ID, TITLE, INTELLI_RATING, STATUS, NICKNAME " +
						"FROM "+url+" WHERE DEPARTMENT IN " + cats + " ORDER BY REF_ID DESC LIMIT " + rowq + ", 20");
				rs = ps.executeQuery();
				request.setAttribute("row", row1);
