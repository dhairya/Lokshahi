      ...
      <div class="main-content">  <div id="translation"></div>
        ...
        <table width="200%" border="1">
          ...
          <tr>
            ...
            <td><a href="/CMS/Funds_view?consti=<%=consti%>&ward=<%=ward%>&fundid=<%=rs.getLong("FUND_ID")%>"><%=rs.getString("TITLE")%></a></td>
		...
		<br /><br />
		<a href="/CMS/<%=session.getAttribute("Access").toString().charAt(3)%>/funds_add.jsp?consti=<%=consti%>&ward=<%=ward%>">Add Funds</a>
        <br /><br />
		<%if(request.getAttribute("row").equals("1")){ %>
          Previous
          <%}else{ %>
		  <a href = "/CMS/Funds_prev?consti=<%=consti%>&ward=<%=ward%>&row=<%=request.getAttribute("row")%>">Previous</a>
		  <%} %>
		  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		  <%if(i==20 && rs.next()){ %>
		  <a href = "/CMS/Funds_nxt?consti=<%=consti%>&ward=<%=ward%>&row=<%=request.getAttribute("row")%>">Next</a>
          <%}else{ %>
          Next
          <%} %>
        </div>
